...
3) Finally, it calculates NMDS ordinations and ordination plots for all singlepools. 
...
Import singlepool OTU tables. These tables include all OTUs, not just Arthropoda-assigned OTUs. 
...
plot Procrustes plots for A vs B
optimal Ta and optimal cycle number
```{r procrustes A B}
...
(s1 <- sd(correlationsCD_sametags)/sqrt(length(correlationsCD_sametags)))

correlationsCD_difftags
(n1 <- length(correlationsCD_difftags))
(m2 <- mean(correlationsCD_difftags))
(s2 <- sd(correlationsCD_difftags)/sqrt(length(correlationsCD_difftags)))

(CDsinglepoolstest <- t.test2(m1, m2, s1, s2, n1, n2))
```

plot Procrustes plots for G and H (omit H1)
Touchdown PCR
```{r procrustes G H}
# protestG1H1 <- protest(ordinations$G1, ordinations$H1) # H1 cannot be used because is is missing one sample hhhlleg
protestG2H2 <- protest(ordinations$G2, ordinations$H2)
protestG3H3 <- protest(ordinations$G3, ordinations$H3)


# different tags at higher than optimal temperature, n = 12 such comparisons
# G1:G2, G1:G3, G2:G3, G1:H2, G1:H3, G2:H1, G2:H3, G3:H1, G3:H2, H1:H2, H1:H3, H2:H3
# procrustes tests
...
pairwiseGH_difftags <- c("G1G2", "G1G3", "G2G3", "G1H2", "G1H3", "G2H3", "G3H1", "G3H2", "H2H3")
...
(s1 <- sd(correlationsGH_sametags)/sqrt(length(correlationsGH_sametags)))

correlationsGH_difftags
(n2 <- length(correlationsGH_difftags))
(m2 <- mean(correlationsGH_difftags))
(s2 <- sd(correlationsGH_difftags)/sqrt(length(correlationsGH_difftags)))
